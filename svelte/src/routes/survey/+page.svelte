<script>
    ...
    let title = "架空セミナー「DeltaX概要」アンケート"
    ...
    async function postMessage() {
      ...
      const response = await fetch("/surveybot", {
        method: "POST",
        headers: {
    ...
    async function makeSummary() {
      ...
      const response = await fetch("/surveybot/summary", {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
...
<div class="main">
  ...
  <div class="summary_main">
    <button class="button_summary" on:click={makeSummary}>アンケート提出</button>
    {#if summary_loading}
      <div class="loader"></div>
    {/if}
